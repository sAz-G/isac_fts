function [S_m, V_m, xi_m, delta_m, CRB_iter, R_iter] = optimize_m_debug(E_m, s_c,S_hover, S_total,s_t,s_s, params)
...
CRB_iter = nan(1,1);       % store crb values of each iteration 
R_iter   = nan(1,1);       % store rate values of each iteration 

%% Debug parameter 

S_mat     = zeros(2, N_stg, 1); % store trajectory results of each iteration
V_mat     = zeros(2, N_stg, 1); % store velocity results of each iteration
delta_mat = zeros(N_stg, 1);    % store delta results of each iteration 
xi_mat    = zeros(N_stg, 1);    % store xi results of each iteration 
...
if ~isnan(S)
    ...
    S_mat(:, :, u) = S;      % store trajectory results of each iteration 
    ...
    R_iter(1,u)    = R_taylor;         % store rate results of each iteration 
    CRB_iter(1,u)  = CRB_taylor;       % store crb results  of each iteration 
    
    % return output variables. This is the final optimization result
    S_m         = S_mat(:,:,1:u);      % optimal trajectory 
    V_m         = V_mat(:,:,1:u);      % optimal velocity
    xi_m        = xi_mat(:,1:u);     % optimal xi
    delta_m     = delta_mat(:,1:u);  % optimal delta
else
    return
