...
function results_M = multi_stage(params, setup)
    ...
    mu = params.sim.mu;        % Measurement step. Measure sensing target every mu steps
    M = 7;                     
    ...
    E_m_vec = nan(M, 1);                     % Store the energy available at each stage 
    ...
    CRB_opt_vec = nan(M, 1);                 % Store the optimal CRB solutions 
    ...
    while E_min < E_m % Break if the energy is not enough for additional N_stg points
        ...
        D_meas(:, m) = sense_target(s_t, S_opt_mat(:, mu:mu:end, m), params); % Get new K_stg measurements
